...
resourcename="grizzly"  # lower case, only
resourcegroup="grizzly_rg"
...
# git clone https://github.com/apilogicserver/grizzly.git
# cd grizzly
...
az group create --name grizzly_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group grizzly_rg --sku S1 --is-linux
az container create --resource-group grizzly_rg --name grizzly --image apilogicserver/grizzly:latest --dns-name-label grizzly --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//grizzly.westus.azurecontainer.io:5656'
